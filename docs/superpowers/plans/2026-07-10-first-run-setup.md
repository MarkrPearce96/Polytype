...
**Goal:** A 6-step SwiftUI onboarding window shown on first launch (Welcome → Accessibility → Key → Language → Shortcuts → Done), reopenable from the menu, that reuses existing app pieces and never re-nags.

**Architecture:** One new file `SetupWizard.swift` (a `SetupView` with step state + a `SetupWindowController` + a `setupCompleted` flag). `AppDelegate` shows it on first launch and adds a "Setup Assistant…" menu item. No TranslationCore changes; verified live (it's UI).

**Tech Stack:** Swift 5.9, SwiftUI/AppKit. macOS 14 floor.

## Global Constraints

- `swift-tools-version: 5.9`; floor `.macOS(.v14)`; zero compiler warnings.
- Reuse existing pieces unchanged: `HotkeyRecorder`/`HotkeyAccess`, `LanguagePrefs`/`Languages`, `KeychainSecretStore`/`googleKeyName`, `LoginItem`, `AXIsProcessTrusted`.
- `setupCompleted` persists in `UserDefaults` (key `"setupCompleted"`); set on first dismissal (finish or window close), so the wizard shows once.
- SwiftUI `@State` seeded from `@MainActor` shared state in `.onAppear`, never in the initializer.
- App build: `swift build --package-path TypeTranslatorApp`.

---

### Task 1: SetupWizard.swift (the wizard)

**Files:**
- Create: `TypeTranslatorApp/Sources/TypeTranslatorBar/SetupWizard.swift`

**Interfaces:**
- Produces: `@MainActor enum SetupState { static var completed: Bool }`; `@MainActor final class SetupWindowController { static let shared; func show() }`; `struct SetupView`.

- [ ] **Step 1: Create SetupWizard.swift**
...
```bash
git add TypeTranslatorApp/Sources/TypeTranslatorBar/SetupWizard.swift
git commit -m "feat: first-run setup wizard (SetupView + window controller)"
```

---

### Task 2: Show on first launch + menu item

**Files:**
- Modify: `TypeTranslatorApp/Sources/TypeTranslatorBar/AppDelegate.swift`

**Interfaces:**
- Consumes: `SetupState.completed`, `SetupWindowController.shared.show()`.

- [ ] **Step 1: Add the "Setup Assistant…" menu item**

In the menu construction (near the `settingsItem`), add before it:
...
- [ ] **Step 2: Show the wizard on first launch, and don't double-prompt for Accessibility**

`applicationDidFinishLaunching` already fires a standalone Accessibility system prompt:

```swift
_ = AXIsProcessTrustedWithOptions(["AXTrustedCheckOptionPrompt": true] as CFDictionary)
```

Change that line so it only prompts returning users (the wizard's Accessibility step covers first-run):
...
Then at the very end of `applicationDidFinishLaunching`, add:
...
- [ ] **Step 4: Manual verify (controller/user)**

Run `./scripts/build-bar.sh`, then clear the flag and launch:
`defaults delete com.typetranslator.bar setupCompleted 2>/dev/null; killall TypeTranslatorBar 2>/dev/null; open "build/Type Translator.app"`.
- The wizard appears on launch; walk through all 6 steps.
- The Accessibility step's Next stays disabled until permission is granted, then the pill turns green and Next enables.
- Key/language/shortcuts entered persist (check Settings shows them).
- After finishing, relaunch → wizard does NOT reappear; the "Setup Assistant…" menu item reopens it.

- [ ] **Step 5: Commit**

```bash
git add TypeTranslatorApp/Sources/TypeTranslatorBar/AppDelegate.swift
git commit -m "feat: show setup wizard on first launch + Setup Assistant menu item"
```
...
- [ ] **Step 1: Document the setup wizard**

In `README.md` (near Setup), note that on first launch a setup assistant walks you through Accessibility permission, your Google key, language, and shortcuts, and can be reopened anytime from the menu ("Setup Assistant…").

- [ ] **Step 2: Commit**

```bash
git add README.md
git commit -m "docs: document the first-run setup assistant"
```
