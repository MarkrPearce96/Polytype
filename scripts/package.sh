...
# The result (dist/Type-Translator-<version>.dmg) can be attached to a GitHub
...
DMG="$DIST/Type-Translator-$VERSION.dmg"
STAGING="$(mktemp -d)"
...
Type Translator — install

1. Drag "Type Translator.app" onto the Applications folder (shown here).

2. FIRST launch only: because this app isn't notarized by Apple, macOS will
   block a normal double-click. Instead:
     • Right-click (or Control-click) "Type Translator" in Applications
   ...
   (Alternatively, run this once in Terminal:
      xattr -dr com.apple.quarantine "/Applications/Type Translator.app" )

3. Grant Accessibility permission when asked (System Settings ▸ Privacy &
   Security ▸ Accessibility ▸ enable "Type Translator"). This lets the hotkey
...
mkdir -p "$DIST"
rm -f "$DMG"
hdiutil create -volname "Type Translator" -srcfolder "$STAGING" \
